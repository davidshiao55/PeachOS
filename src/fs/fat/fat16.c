...
struct fat_directory {
    struct fat_directory_item *item; // point to first item in the fat directory
    int total;
    int sector_pos;
...
int fat16_read(struct disk *disk, void *descriptor, uint32_t size, uint32_t nmenb, char *out_ptr);
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private, struct fat_directory *out_directory)
{
    ...
    // array of fat_directory_item in the root directory
...
int fat16_resolve(struct disk *disk)
{
    ...
    // Binding the disk to the fat16 driver
    ...
    // Check if header has FAT16 signature
...
static int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++) {
        ...
        // Check if the secotr is marked as bad
...
static int fat16_read_internal_from_stream(struct disk *disk, struct disk_stream *stream, int cluster, int offset, int total, void *out)
{
    ...
    // Only read one cluster at a time
...
void fat16_free_item(struct fat_item *item)
{
    if (item->type == FAT_ITEM_TYPE_DIRECTORY) {
        fat16_free_directory(item->fat_directory);
...
struct fat_directory *fat16_load_fat_directory(struct disk *disk, struct fat_directory_item *item)
{
    ...
    // Get the first secotr of the item
...
struct fat_item *fat16_new_fat_item_for_directory_item(struct disk *disk, struct fat_directory_item *item)
{
    ...
    if (item->attribute & FAT_FILE_SUBDIRECTORY) {
        f_item->fat_directory = fat16_load_fat_directory(disk, item);
        f_item->type = FAT_ITEM_TYPE_DIRECTORY;
    } else {
        f_item->type = FAT_ITEM_TYPE_FILE;
        // Clone the item to prevent freeing the given item won't crash
...
struct fat_item *fat16_find_item_in_directory(struct disk *disk, struct fat_directory *directory, const char *name)
{
    ...
    // loop through every item in directory
...
struct fat_item *fat16_get_directory_entry(struct disk *disk, struct path_part *path)
{
    ...
    // Traverse item in the path
    while (next_part != 0) {
        if (current_item->type != FAT_ITEM_TYPE_DIRECTORY) {
            current_item = 0;
            break;
        }
        struct fat_item *tmp_item = fat16_find_item_in_directory(disk, current_item->fat_directory, next_part->part);
        // Free the old one
...
int fat16_stat(struct disk *disk, void *private, struct file_stat *stat)
{
    ...
    // Currently does not allow stating directory
